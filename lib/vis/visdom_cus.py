...
class VisTracking(VisBase):
    ...
    def save_data(self, data, **kwargs):
        ...
        # if isinstance(data[-3], torch.Tensor):
        #     # 增加了 logits_mask 的参数
        #     # self.logits_mask = data[-4]
        
        self.frame_id = data[-3]
        self.search_factor = data[-2]
        self.flag = data[-1]
        boxes_masks = boxes_masks[:-3]
    ...
    def draw_data(self):
        ...
        if isinstance(disp_image, torch.Tensor):
            disp_image = disp_image.permute(1, 2, 0).cpu().numpy()
            
        # # 更新的显示信息
        if len(boxes) >= 2:
            iou = compute_iou(boxes[0], boxes[1])
            # iou_text = f'IoU: {iou:.2f}, gt:{boxes[0]}'
            
            gt_np = np.round(boxes[0].numpy(), 2)
            box_str = ','.join(map(str, gt_np))
            
            iou_text = 'IOU:{}'.format(np.round(iou,2)) 
            
            
        if self.frame_id is not None:
            frame_id_text = 'ID:{:4d}'.format(self.frame_id)
            
        if self.search_factor is not None and self.flag is not None:
            search_factor_text = 'SF:{:.1f}-{:.2f}'.format(self.search_factor,self.flag)
        
        # if self.logits_mask is not None:
        #     logits_mask_text = 'M:{:.2f}'.format(self.logits_mask)
        
        cv2.putText(disp_image, '{}-{}-{}'.format(frame_id_text, iou_text, search_factor_text), (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2, cv2.LINE_AA)
