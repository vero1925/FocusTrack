...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def __init__(self, datasets, p_datasets, samples_per_epoch, max_gap,
                 ...
                 train_cls=False, pos_prob=0.5, seed: int=0):
        """
        args:
        ...
        # 引入随机种子
        self._EXT_SEED_STEP = 30011  # better to be a prime number
        self._SEED_STEP = 10007  # better to be a prime number
        self._SEED_DIVIDER = 1000003  # better to be a prime number
        self.ext_seed = seed
        
        self.rng = np.random.RandomState(seed)   # seed是传入的
        self.positive_pair_ratio = pos_prob
        self.negative_pair_ratio = 1.0-pos_prob
    ...
    def __getitem__(self, index):
        ...
        seed = (torch.initial_seed() + index * self._SEED_STEP +
        ...
        if self.train_cls:
            ...
            return self.getitem(rng)

    def getitem(self, rng):
        """
        returns:
            TensorDict - dict containing all the data blocks
        """
        is_negative_pair = (rng.rand() < self.negative_pair_ratio)
        ...
        while not valid:
            ...
            if is_negative_pair:
                if is_video_dataset:
                    ...
                    # Sample two sequences with enough visible frames                
                    seq_id, visible, seq_info_dict = self.sample_two_seqs_from_dataset(dataset, is_video_dataset)
                    
                    template_frame_ids = self._sample_visible_ids(visible[0], 
                                                                num_ids=1, 
                                                                min_id=0,
                                                                max_id=len(visible[0]))
                    search_frame_ids = self._sample_visible_ids(visible[1], 
                                                                num_ids=1, 
                                                                min_id=0,
                                                                max_id=len(visible[1]))
                    
                    template_frames, template_anno, meta_obj_train = dataset.get_frames(seq_id[0], template_frame_ids, seq_info_dict[0])
                    search_frames, search_anno, meta_obj_test = dataset.get_frames(seq_id[1], search_frame_ids, seq_info_dict[1])
                    
                    H_t, W_t, _ = template_frames[0].shape
                    # template_masks = template_anno['mask'] if 'mask' in template_anno else [torch.zeros((H_t, W_t))] * self.num_template_frames
                    template_masks = template_anno['mask'] if 'mask' in template_anno else \
                                self.generate_mask_from_bbox(template_frames[0].shape[:2], template_anno['bbox'])
                    
                    
                    H_s, W_s, _ = search_frames[0].shape
                    # search_masks = search_anno['mask'] if 'mask' in search_anno else [torch.zeros((H_s, W_s))] * self.num_search_frames
                    search_masks = search_anno['mask'] if 'mask' in search_anno else \
                                self.generate_mask_from_bbox(search_frames[0].shape[:2], search_anno['bbox'])
                ...
                try:
                    ...
                    if dataset.name in ['got10k', 'trackingnet', 'lasot']:
                        data = self.processing_vis(data)
                    else:
    ...
    def generate_mask_from_bbox(self, shape, bboxes):
        ...
        for bbox in bboxes:
            ...
            xmax = int(min(width, xmax.item()))
            ymax = int(min(height, ymax.item()))
    ...
    def sample_two_seqs_from_dataset(self, dataset, is_video_dataset):
        # Sample two sequences with enough visible frames
        enough_visible_frames = [False, False]
        seq_info_dicts = [None, None]
        visibles = [None, None]
        
        while not all(enough_visible_frames):  # While both sequences don't satisfy the condition
            # Sample two different seq_ids
            seq_ids = random.sample(range(dataset.get_num_sequences()), 2)
            
            for i, seq_id in enumerate(seq_ids):
                # Sample frames for each sequence
                seq_info_dict = dataset.get_sequence_info(seq_id)
                visible = seq_info_dict['visible']
                
                enough_visible_frames[i] = visible.type(torch.int64).sum().item() > 2 * (
                    self.num_search_frames + self.num_template_frames) and len(visible) >= 20
                
                enough_visible_frames[i] = enough_visible_frames[i] or not is_video_dataset
                
                # Store the information for each sequence
                if enough_visible_frames[i]:
                    visibles[i] = visible
                    seq_info_dicts[i] = seq_info_dict
        
        return seq_ids, visibles, seq_info_dicts
