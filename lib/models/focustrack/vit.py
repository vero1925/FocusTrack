...
from lib.models.focustrack.base_backbone import BaseBackbone
from lib.models.focustrack.utils import combine_tokens, recover_tokens
...
class VisionTransformer(BaseBackbone):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=2, embed_dim=768, depth=12,
                 ...
                 add_cls_token=False, use_cls_token=False,
                 out_indices=[5, 7, 11],):
        """
        Args:
    ...
    def forward_features(self, z, x):
        ...
        for i, blk in enumerate(self.blocks):
            x = blk(x)
            if (self.out_indices is not None) and (i in self.out_indices[:-1]): 
                intermediate_outputs.append(x[:,-lens_x:])
        ...
        if (self.out_indices is not None) and (i in self.out_indices):
            intermediate_outputs.append(out[:,-lens_x:])
 
        aux_dict = {"attn": None,
                    "intermediate_outputs": intermediate_outputs,}
        
        if self.add_cls_token and self.use_cls_token:
            aux_dict["before_norm"] = x
