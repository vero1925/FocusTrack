...
def _save_tracker_output(seq: Sequence, tracker: Tracker, output: dict):
    ...
    if seq.dataset in ['trackingnet', 'got10k_val', 'got10k_test','lasot', 'tnl2k', 'lasot_extension_subset', 'otb', 'got10k','antiuav410']:
...
def run_sequence(seq: Sequence, tracker: Tracker, debug=False, num_gpu=8):
    ...
    def _results_exist():
        ...
        if seq.object_ids is None:
            if seq.dataset in ['trackingnet', 'got10k_test', 'got10k_val','lasot', 'tnl2k', 'lasot_extension_subset', 'otb', 'antiuav410_test', 'antiuav410']:
                base_results_path = os.path.join(tracker.results_dir, seq.dataset, seq.name)
                bbox_file = '{}.txt'.format(base_results_path)
            else:
...
def run_dataset(dataset, trackers, debug=False, threads=0, num_gpus=8):
    ...
    if mode == 'sequential':
        ...
        param_list = [(seq, tracker_info, debug, num_gpus) for tracker_info, seq in product(trackers, dataset)]
...
if mode == 'sequential':
        for seq in tqdm(dataset, desc="Processing Sequences", unit="sequence"):
            for tracker_info in trackers:
                run_sequence(seq, tracker_info, debug=debug)
    elif mode == 'parallel':
        param_list = [(seq, tracker_info, debug, num_gpus) for tracker_info, seq in product(trackers, dataset)]
        with mp.Pool(processes=threads) as pool:
            list(tqdm(pool.imap(run_sequence, param_list), total=len(param_list), desc="Processing Parallel", unit="task"))

    print('Done, total time: {}'.format(str(timedelta(seconds=(time.time() - dataset_start_time)))))


if mode == 'sequential':
        # Add progress bar for sequential mode
        for seq in tqdm(dataset, desc="Processing sequences", unit="sequence"):
            for tracker_info in trackers:
                run_sequence(seq, tracker_info, debug=debug)
    elif mode == 'parallel':
        ...
        # Use tqdm to show progress in parallel mode
        with multiprocessing.Pool(processes=threads) as pool:
            # starmap gives us an easy way to pass multiple arguments to run_sequence
            list(tqdm(pool.imap_unordered(run_sequence, param_list), total=len(param_list), desc="Processing in parallel", unit="task"))
