...
# from lib.train.dataset.antiuav410 import AntiUAV410
from lib.test.analysis.antiuav410 import AntiUAV410
...
class ExperimentAntiUAV410(object):
    ...
    def run(self, tracker, visualize=False):
        ...
        print('[Overall] Mixed Measure: %.01f\n' % (np.mean(overall_performance).item() * 100))
    ...
    def report(self, trackers, plot_curves=True, plot_attcurves=True):
        ...
        for trackerid, tracker in enumerate(trackers):
            ...
            succ_curve = np.zeros((seq_num, self.nbins_iou)) # 21
            prec_curve = np.zeros((seq_num, self.nbins_ce))  # 51
        ...
        for ii in range(len(overall_SAs)):
            text = '[Overall] %20s %20s: %.01f' % (trackers[ii]['name'], 'State accuracy', overall_SAs[ii]*100)
            with open(sa_file, 'a', encoding='utf-8') as f:
                f.write(text)
                f.write('\n')
            print(text)
    ...
    def plot_curves(self, tracker_names):
        ...
        for i, name in enumerate(tracker_names):
            ...
            if name == "Siam R-CNN":
                ...
                success_score = performance[name][key]['success_score'] * 100
                legends.append('[%.1f] %s' % (success_score, name))
        ...
        legend = ax.legend(lines, legends, loc='best', ncol=2, frameon=False, fontsize=8)
        
        # legend = ax.legend(lines[:1:], legends[:1:], loc='best', ncol=2, frameon=False, fontsize=8)
        # if len(legend.get_texts()) > 0:
        #     legend.get_texts()[0].set_weight('bold')  # 根据需调整加粗的条目索引
        ...
        for i, name in enumerate(tracker_names):
            ...
            if name == "Siam R-CNN":
                ...
                precision_score = performance[name][key]['precision_score'] * 100
                legends.append('[%.1f] %s' % (precision_score, name))
    ...
    def plot_attcurves(self, tracker_names, att_name, att_key):
        ...
        for i, name in enumerate(tracker_names):
            ...
            if name == "Siam R-CNN":
                ...
                att_success_score = performance[name][att_key]['att_success_score'] * 100
                legends.append('[%.1f] %s' % (att_success_score, name))
        ...
        for i, name in enumerate(tracker_names):
            ...
            if name == "Siam R-CNN":
                ...
                att_precision_score = performance[name][att_key]['att_precision_score'] * 100
                legends.append('[%.1f] %s' % (att_precision_score, name))
