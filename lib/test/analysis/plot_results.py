...
def check_and_load_precomputed_results(trackers, dataset, report_name, tracker_param, tracker_name, force_evaluation=True, **kwargs):
    # Load data
    settings = env_settings()

    # Load pre-computed results
    result_plot_path = os.path.join(settings.result_plot_path, tracker_name, tracker_param, report_name)
...
def print_results(trackers, dataset, arg_list, merge_results=False,
    ...
    """
    # Load pre-computed results
    report_name = arg_list[0]   # name of dataset
    tracker_param = arg_list[1]
    tracker_name = arg_list[2]
    eval_data = check_and_load_precomputed_results(trackers, dataset, report_name, tracker_param, tracker_name, **kwargs)
    ...
    print('Reporting results over {} / {} sequences'.format(valid_sequence.long().sum().item(), valid_sequence.shape[0]))
